...
io.on('connection', (socket) => {
    ...
    //===Show a specific board card if the player has any problem with any card

    socket.on('show_board_card',(cardPosition)=>{
      cards= JSON.parse(db.get("board_card"));
      let itemToSend =cards.filter(x=>{
        return x[0]==cardPosition;
      });
      if(itemToSend.length>0){
        socket.emit("board-card-revealed",itemToSend[0]);
      }
    });

    socket.on('show_my_card', (username) => {
      card=JSON.parse(db.get(`${username}-card`));
      socket.emit('your_card_is',username);
    });
    ...
    socket.on('start_game_admn',(amounToBeGiven)=>{      
      ...
      if(JSON.parse(db.get("final_result")).length>0){
        ...
        playerCardInfo.forEach(x=>{          
          ...
          console.log(card2);
          BlockThisCard(card2[0],card2[1]);
          db.set(`${x.name}-card`,JSON.stringify([[card[0],card[1]],[card2[0],card2[1]]]));
          io.emit("mynewcards",[[card[0],card[1]],[card2[0],card2[1]]],x.name);
        ...
        playerList.forEach(x => {          
          ...
          db.set(`${x.name}`, JSON.stringify(obj));
